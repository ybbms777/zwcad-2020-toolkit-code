param([ValidateSet('dimensions','trim','extend','tolerance','symbols','font','dimline','frame','tke','all','fonts')][string]$Choice)
$ErrorActionPreference='Stop'
[Console]::OutputEncoding=[Text.UTF8Encoding]::new($false)
$names=@{dimensions='智能标注';trim='快速修剪';extend='快速延伸';tolerance='几何公差';symbols='机械符号';font='改字体';dimline='标注线属性';frame='标题框缩放';tke='图框编辑器'}
...
$allModules=@('dimensions','trim','extend','tolerance','symbols','font','dimline','frame','tke')
# Weights are cosmetic only -- they shape how long the progress bar runs,
# not the actual (near-instant) install work.
$weights=@{dimensions=3;trim=3;extend=3;tolerance=1;symbols=2;font=4;dimline=3;frame=1;tke=2}
$stages=@(
...
function Show-Menu {
    ...
    Write-Host '    [9] 图框编辑器      ' -NoNewline -ForegroundColor Yellow
    Write-Host 'TKE，料号只输一次数字，自动填 客户编号/产品编号/产品料号 三处（仅样品图）' -ForegroundColor Gray
    Write-Host ''
    Write-Host '    [10] 全部安装       ' -NoNewline -ForegroundColor Green
    Write-Host '★ 推荐 — 一次装齐以上全部功能' -ForegroundColor Green
    Write-Host '    [11] 字体安装       ' -NoNewline -ForegroundColor Yellow
...
if(-not $Choice){
    ...
    do {
        ...
        $valid=$inputValue -match '^\s*(11|10|[1-9])(\s*[,，]\s*(11|10|[1-9]))*\s*$'
        if(-not $valid){Write-Host '请输入 1-11，或用逗号分隔多个编号。' -ForegroundColor Yellow}
    } until($valid)
    $numbers=@($inputValue -split '[,，]' | ForEach-Object {$_.Trim()})
    $fontMode=$numbers -contains '11'
    $modNumbers=@($numbers | Where-Object {$_ -ne '11'})
    # 编号必须和上面菜单显示一致：[5]改字体 [6]标注线属性 [7]机械符号 [8]标题框缩放 [9]图框编辑器
    # （原来 5 和 7 是反的，选 [5]改字体 实际装的是机械符号）
    $numberMap=@{'1'='dimensions';'2'='trim';'3'='extend';'4'='tolerance';'5'='font';'6'='dimline';'7'='symbols';'8'='frame';'9'='tke'}
    if($modNumbers -contains '10'){$selected=@($allModules)}
